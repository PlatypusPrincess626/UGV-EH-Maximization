...
class UGVSimulator:
    ...
    # Period-initial state of charge.
    #
    # Training uses 0.30-0.40. The certification probe widens it so the
    # set of period-initial states contains the end-of-period states,
    # measured at 0.401-0.909, which is the closure hypothesis the
    # multi-period result needs: without it, a guarantee proved for one
    # period cannot be chained to the next, because the next period
    # begins outside the distribution the first was certified on.
    #
    # Overridable rather than changed outright: a trained policy must
    # be evaluated on the start distribution it was trained on wherever
    # the ablation and baseline numbers are concerned, and only the
    # closure probe departs from it.
    START_SOC_MIN = float(os.environ.get("LTAC_START_SOC_MIN", "0.30"))
    START_SOC_MAX = float(os.environ.get("LTAC_START_SOC_MAX", "0.40"))
